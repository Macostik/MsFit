...
//  Ms Fit
//
//  Created by Yura Granchenko on 16.12.2019.
...
open class YouTubePlayerView: UIView, UIWebViewDelegate {
    ...
    public static var shared: YouTubePlayerView {
        let videoPlayer = YouTubePlayerView(frame: .zero)
        let heightConstraints = videoPlayer.heightAnchor
            .constraint(equalToConstant: Constants.sH_812 ? Constants.sW * 0.68 : Constants.sW * 0.63)
        heightConstraints.isActive = true
        videoPlayer.playerVars = ["controls": 2,
                                  "playsinline": 1,
                                  "autohide": 1,
                                  "showinfo": 1,
                                  "autoplay": 1,
