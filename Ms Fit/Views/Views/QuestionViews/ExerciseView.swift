...
class ExerciseView: UIView {
    ...
    private let mealTableView = specify(UITableView(), {
        ...
        $0.register(QuestionExerciseCell.self,
                    forCellReuseIdentifier: QuestionExerciseCell.identifier)
        $0.rowHeight = 40
    ...
    fileprivate func setupUI() {        
        Observable.just(QuestionExerciseModels.allCases).bind(to: mealTableView.rx
            .items(cellIdentifier: QuestionExerciseCell.identifier,
                   cellType: QuestionExerciseCell.self)) { _, model, cell in
...
class QuestionExerciseCell: UITableViewCell, CellIdentifierable {
    ...
    public func setup(meals: QuestionExerciseModels) {
